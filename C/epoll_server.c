...
/* logger.c/health.c への接続を保持するグローバル変数は、仮に標準出力に設定 */
int logger_fd = STDOUT_FILENO;
int health_fd = STDOUT_FILENO;
    ...
    while (1) {
        ...
        for (int i = 0; i < nfds; i++) {
            if (events[i].data.fd == listenfd) {
                ...
                dprintf(logger_fd, "New client connected\n");
            } else {
                // 既存クライアントからのデータ受信
                ...
                if (n <= 0) {
                    ...
                    close(fd);
                    dprintf(logger_fd, "Client disconnected\n");
                } else {
			char cmd = buf[0];
			switch (cmd) {
			case 'P':
                            // loggerを先に起床させてセマフォを取得させる（インバージョン誘発のため）
			    dprintf(logger_fd, "PAYING %.*s", n-1, buf+1); // Payment 過程をloggerに登録
			 ...
			 case 'L':
			    // loggerへ
			    dprintf(logger_fd, "LOGGING %.*s", n-1, buf+1); // Logging 過程をloggerに登録
			    write(fd, "ACK: logger\n", 12);
			    break;
